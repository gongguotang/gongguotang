...
[verbose,verboseI,maxIter,optTol,eta1,eta2,gamma1,gamma2,solver,Hessian,cgSolve,...
    HvFunc,useComplex,corrections,interp] = myProcessOptions(options,...
    'verbose',0,'verboseI',0,'MaxIter',250,'optTol',1e-9,...
    'eta1',.25,'eta2',3/4,'gamma1',1/4,'gamma2',2,...
    'solver','schur','Hessian','exact','cgSolve',0,'HvFunc',[],'useComplex',0,...
    'corrections',100,'interp',0);
...
% TR parameters
delta = (1/10)*norm(g); % Initial trust-region radius
delta0 = 1e150;
lambda = 1;
normType = 2;
...
for i = 1:maxIter
    ...
    elseif strcmp(solver,'Linf')
        ...
        if i == 1 || (i == 2 && ~strcmp(Hessian,'exact'))
            ...
            if (exist('qpip','file')==3)
                ...
                useQPIP = 0;
                qpOps = optimset('Display','none','LargeScale','off','MaxIter',100000,'Algorithm','active-set');
            end
        end
    ...
    if interp && ~strcmp(stepType2,'')
        lambda = (-g'*p)/(2*(f_new - f - g'*p));
        delta_new = lambda*norm(p);
        if delta_new < minDelta
            fprintf('Min\n');
            delta = minDelta;
        elseif delta_new > maxDelta
            fprintf('Max\n');
            delta = maxDelta;
        else
            fprintf('Interp\n');
